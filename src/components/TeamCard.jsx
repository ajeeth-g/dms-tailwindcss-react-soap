...
const TeamCard = ({ user }) => {
  ...
  const displayName = user_name || domain_user_name;
  ...
  // Safely compute title-case only when non-empty
  const displayTitle = displayName ? toTitleCase(displayName) : '';
  const designationTitle = designation ? toTitleCase(designation) : '';

  return (
    <Link to="/document-list" state={{ userName: displayName }}>
      <div className="cust-card-group p-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3 flex-shrink-0">
            <div className="w-14 h-14 overflow-hidden rounded-full ring-2 ring-green-500 flex-shrink-0">
              <img
                src={imageSrc || "/placeholder-user.png"}
                alt={displayName}
                title={displayTitle}
                onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = "/placeholder-user.png"; }}
        ...
        <div className="w-full bg-gray-200 rounded-full h-1 mb-1">
          <div className="h-1 rounded-full bg-green-500" style={{ width: `${progressPercent}%` }} />
        </div>
        <p className="text-xs text-gray-500 text-right mb-4">{progressPercent}% Completed</p>
        ...
        <div className="flex items-center justify-between text-xs text-gray-500">
          ...
          <div className="flex space-x-0.5">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star key={i} size={12} className={i < Math.round(monthProgressPercent / 20) ? 'text-yellow-400' : 'text-gray-300'} />
            ))}
          </div>
        </div>
      </div>
